...
public class ButtonTest2 extends Activity implements OnKeyListener, OnClickListener {
    final String TAG = "ButtonTest2";
    List<Integer> mKeys;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String[] parts;
        if (prefs.getString("keys", "") != "") {
            parts = prefs.getString("keys", "").split(",");                             
            for (String s : parts) if (s != null && s != "") mKeys.add(Integer.valueOf(s));
        }
        else initKeys(); //addAllKeys();
            
        if (prefs.getString("skipped", "") != "") {
            parts = prefs.getString("skipped", "").split(",");
            for (String s : parts) if (s != null && s != "") mSkipped.add(Integer.valueOf(s));
        }
//mKeys.add(KeyEvent.KEYCODE_A);           
        setText();
    }
    ...
    void setText() {
        ...
        mKeyTextView.setText(getKeyName(mKeys.get(0)));
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {        
        if (mKeys.isEmpty()) return super.onKeyDown(keyCode, event);   
        int flags = event.getFlags();
        if ((flags & KeyEvent.FLAG_FROM_SYSTEM) == 0
            || (flags & KeyEvent.FLAG_SOFT_KEYBOARD) != 0) return true;

        if (keyCode == mKeys.get(0)) {
            mKeys.remove(0);
            setText();
        } 
...
//}
    ...
    public void onClick(View v) {
        if ((Button)v == mSkipThisButton) { 
            mSkipped.add(mKeys.get(0));
            mKeys.remove(0);
            setText();
        }
        if ((Button)v == mSkipRestButton) {
            for (int i : mKeys) mSkipped.add(i);
            mKeys.clear();
            setText();
        }
        if ((Button)v == mContinueButton) {
            // TODO continue
        }
    }
    ...
    public void onPause() {
        super.onPause();
        SharedPreferences.Editor ed = getSharedPreferences(TAG, 0).edit();
        String s = "";
        for (int i : mKeys) s += String.valueOf(i) + ",";        
        ed.putString("keys", s);
        s = "";
        for (int i : mSkipped) s += String.valueOf(i) + ",";
        ed.putString("skipped", s);
        ed.commit();
    }
    ...
    void initKeys() {
        ...
        //addKey(KeyEvent.KEYCODE_CALL); (can be used, but there are false positives)
    ...
    void addAllKeys() {     
        ...
        addKey(KeyEvent.KEYCODE_CALL);		// cant override
        addKey(KeyEvent.KEYCODE_CAMERA);
        addKey(KeyEvent.KEYCODE_CAPS_LOCK);
        ...
        addKey(KeyEvent.KEYCODE_ENDCALL);		// cant override
        addKey(KeyEvent.KEYCODE_ENTER);
        ...
        addKey(KeyEvent.KEYCODE_HOME);		// cant override
        ...
        addKey(KeyEvent.KEYCODE_PLUS);
        addKey(KeyEvent.KEYCODE_POUND);
        addKey(KeyEvent.KEYCODE_POWER);		// cant override
